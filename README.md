...
- **CLI 开场**：黑屏终端里输入 `/cry` 回车，进入页面；点击任意处或按任意键可跳过，回访走快速通道
- **复古 CRT 电视**：程序化建模的 3D 电视，屏幕是实时渲染的 canvas 终端
- **滚动换台**：向下滚驱动镜头推进电视屏幕，镜头停靠后 "NO SIGNAL" 雪花黑场切台，直接落到 CH 01（不再露出屏幕内部）
...
首页与关于子页均已完成。精选作品收录六个真实项目（暗日地下城 / 拼豆 / 血与火 / 浴血黑帮 / 人生进度条 / 辩论赛计时器）。电脑端为"环形展台"：钉住后先用开机式撑开把舞台画面从一条待机亮线上下打开，六张圆角海报（青色对位角标 + 正前位 ON AIR + 按作品色温的光晕）站上带俯角与鼠标视差的 3D 圆环，立在透视地板上，顶光锥里飘着灰尘；竖向滚动 / 横向拖拽 / 滚轮 / ←→ 驱动环旋转，顶光固定打在正前位（转进光锥的被照亮、背面透出电视机背板），吸附对正时光束回弹 + 底部索引条跳号，正前作品的说明与源码固定显示在台下说明档；移动端与 reduced-motion 竖向堆叠。整张海报可点击跳转 demo 或源码。

已部署于 Vercel：<https://crt-folio.vercel.app>。字体自托管，支持 `#works` / `#footer` / `#about` 深链。
